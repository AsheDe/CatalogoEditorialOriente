...
public class RevealFAB extends FrameLayout {
    ...
    public void recogerAnimacion() {
        float inicial = Math.max(fabLayout.getWidth(), fabLayout.getHeight());
        ...
        Animator.AnimatorListener listener = new Animator.AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                //  View vista =   LayoutInflater.from(context).inflate(R.layout.vistainflable,null);
            }
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
            Animator animator = ViewAnimationUtils.createCircularReveal(fabLayout, x, y, inicial, 0);
            fabLayout.setVisibility(INVISIBLE);
            animator.addListener(listener);
            animator.start();
        } else {
            SupportAnimator animator =
                    cu.editorialoriente.catalogo.ViewAnimationUtils.createCircularReveal(fabLayout, x, y, inicial, 0);
            fabLayout.setVisibility(INVISIBLE);
    ...
    private void animateFabReveal(float initialRadius, float finalRadius) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
            ...
            SupportAnimator animator =
                    cu.editorialoriente.catalogo.ViewAnimationUtils.createCircularReveal(fabLayout, x, y, initialRadius, finalRadius);
            fabLayout.setVisibility(VISIBLE);
            animator.addListener(new SupportAnimator.AnimatorListener() {
                @Override
                public void onAnimationStart() {
